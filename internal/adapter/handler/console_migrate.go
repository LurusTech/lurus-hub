...
func MigrateConsoleSetting(c *gin.Context) {
	...
	if v := valMap["ApiInfo"]; v != "" {
		...
		if err := json.Unmarshal([]byte(v), &arr); err == nil {
			...
			repo.UpdateOption("console_setting.api_info", string(bytes))
		}
		repo.UpdateOption("ApiInfo", "")
	}
	// Announcements 直接搬
	if v := valMap["Announcements"]; v != "" {
		repo.UpdateOption("console_setting.announcements", v)
		repo.UpdateOption("Announcements", "")
	}
	// FAQ 转换
	if v := valMap["FAQ"]; v != "" {
		var arr []map[string]interface{}
		if err := json.Unmarshal([]byte(v), &arr); err == nil {
			...
			repo.UpdateOption("console_setting.faq", string(bytes))
		}
		repo.UpdateOption("FAQ", "")
	}
	...
	if url != "" && slug != "" {
		...
		repo.UpdateOption("console_setting.uptime_kuma_groups", string(bytes))
	}
	// 清空旧键内容
	if url != "" {
		repo.UpdateOption("UptimeKumaUrl", "")
	}
	if slug != "" {
		repo.UpdateOption("UptimeKumaSlug", "")
	}

	// 删除旧键记录
	oldKeys := []string{"ApiInfo", "Announcements", "FAQ", "UptimeKumaUrl", "UptimeKumaSlug"}
	repo.DB.Where("key IN ?", oldKeys).Delete(&repo.Option{})
