...
func AdminCreateInviteCodes(c *gin.Context) {
	...
	codes, codeStrings, err := repo.CreateInvitationCodes(createdBy, req.Count, req.ExpiresIn)
	if err != nil {
...
func AdminListInviteCodes(c *gin.Context) {
	...
	codes, total, err := repo.GetAllInvitationCodes(startIdx, pageSize)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func AdminSearchInviteCodes(c *gin.Context) {
	...
	codes, total, err := repo.SearchInvitationCodes(keyword, startIdx, pageSize)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func AdminDeleteInviteCode(c *gin.Context) {
	...
	err = repo.DeleteInvitationCode(id)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": err.Error(),
...
func AdminGetInviteCodeStats(c *gin.Context) {
	total, used, expired, active, err := repo.GetInvitationCodeStats()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func AdminCleanupExpiredInviteCodes(c *gin.Context) {
	deleted, err := repo.DeleteExpiredInvitationCodes()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func ValidateInviteCode(c *gin.Context) {
	...
	valid := repo.ValidateInvitationCode(code)
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"valid":   valid,
	})
