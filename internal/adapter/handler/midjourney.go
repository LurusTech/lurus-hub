...
func UpdateMidjourneyTaskBulk() {
	...
	for {
		...
		tasks := repo.GetAllUnFinishTasks()
		if len(tasks) == 0 {
			continue
		}
		...
		taskM := make(map[string]*repo.Midjourney)
		...
		if len(nullTaskIds) > 0 {
			err := repo.MjBulkUpdateByTaskIds(nullTaskIds, map[string]any{
				"status":   "FAILURE",
				"progress": "100%",
			})
			if err != nil {
		...
		for channelId, taskIds := range taskChannelM {
			...
			midjourneyChannel, err := repo.CacheGetChannel(channelId)
			if err != nil {
				logger.LogError(ctx, fmt.Sprintf("CacheGetChannel: %v", err))
				err := repo.MjBulkUpdate(taskIds, map[string]any{
...
// UpdateMidjourneyTaskBulkWithContext updates Midjourney tasks with context cancellation support.
func UpdateMidjourneyTaskBulkWithContext(ctx context.Context) {
	...
	for {
		select {
		case <-ctx.Done():
			common.SysLog("midjourney task update stopped")
			return
		case <-ticker.C:
...
// updateMidjourneyTasks performs a single round of Midjourney task updates.
func updateMidjourneyTasks(ctx context.Context) {
	tasks := repo.GetAllUnFinishTasks()
	if len(tasks) == 0 {
		return
	}
...
func checkMjTaskNeedUpdate(oldTask *repo.Midjourney, newTask dto.MidjourneyDto) bool {
	if oldTask.Code != 1 {
		return true
	}
...
func GetAllMidjourney(c *gin.Context) {
	...
	// 解析其他查询参数
	queryParams := repo.TaskQueryParams{
...
func GetUserMidjourney(c *gin.Context) {
	...
	items := repo.GetAllUserTask(userId, pageInfo.GetStartIdx(), pageInfo.GetPageSize(), queryParams)
	total := repo.CountAllUserTask(userId, queryParams)
