...
func ensureVendorID(vendorName string, vendorByName map[string]upstreamVendor, vendorIDCache map[string]int, createdVendors *int) int {
	...
	var existing repo.Vendor
	if err := repo.DB.Where("name = ?", vendorName).First(&existing).Error; err == nil {
	...
	if err := repo.VendorInsert(v); err == nil {
		*createdVendors++
...
func SyncUpstreamModels(c *gin.Context) {
	...
	for _, name := range missing {
		...
		var existing repo.Model
		if err := repo.DB.Where("model_name = ?", name).First(&existing).Error; err == nil {
		...
		if err := repo.ModelInsert(mi); err == nil {
			createdModels++
	...
	if len(req.Overwrite) > 0 {
		...
		for _, ow := range req.Overwrite {
			...
			var local repo.Model
			if err := repo.DB.Where("model_name = ?", ow.ModelName).First(&local).Error; err != nil {
				continue
			}
...
func SyncUpstreamPreview(c *gin.Context) {
	...
	// 2) 本地已有模型
	var locals []repo.Model
	if len(upstreamNames) > 0 {
		_ = repo.DB.Where("model_name IN ? AND sync_official <> 0", upstreamNames).Find(&locals).Error
	}
	...
	if len(vendorIDs) > 0 {
		var dbVendors []repo.Vendor
		_ = repo.DB.Where("id IN ?", vendorIDs).Find(&dbVendors).Error
	...
	// 3) 缺失且上游存在的模型
	missingList, _ := repo.GetMissingModels()
	var missing []string
