...
import (
	...
	passkeysvc "github.com/QuantumNous/lurus-api/internal/app/passkey"
...
func PasskeyRegisterBegin(c *gin.Context) {
	...
	credential, err := repo.GetPasskeyByUserID(user.Id)
	if err != nil && !errors.Is(err, repo.ErrPasskeyNotFound) {
		common.ApiError(c, err)
		return
	}
	if errors.Is(err, repo.ErrPasskeyNotFound) {
		credential = nil
	}
...
func PasskeyRegisterFinish(c *gin.Context) {
	...
	credentialRecord, err := repo.GetPasskeyByUserID(user.Id)
	if err != nil && !errors.Is(err, repo.ErrPasskeyNotFound) {
		common.ApiError(c, err)
		return
	}
	...
	passkeyCredential := repo.NewPasskeyCredentialFromWebAuthn(user.Id, credential)
...
func PasskeyDelete(c *gin.Context) {
	...
	if err := repo.DeletePasskeyByUserID(user.Id); err != nil {
		common.ApiError(c, err)
		return
	}
...
func PasskeyLoginFinish(c *gin.Context) {
	...
	handler := func(rawID, userHandle []byte) (webauthnlib.User, error) {
		// 首先通过凭证ID查找用户
		credential, err := repo.GetPasskeyByCredentialID(rawID)
		if err != nil {
			return nil, fmt.Errorf("未找到 Passkey 凭证: %w", err)
		}

		// 通过凭证获取用户
		user := &repo.User{Id: credential.UserID}
	...
	// 更新凭证信息
	updatedCredential := repo.NewPasskeyCredentialFromWebAuthn(modelUser.Id, credential)
	...
	if err := repo.UpsertPasskeyCredential(updatedCredential); err != nil {
		common.ApiError(c, err)
		return
	}
...
func AdminResetPasskey(c *gin.Context) {
	...
	if _, err := repo.GetPasskeyByUserID(user.Id); err != nil {
		if errors.Is(err, repo.ErrPasskeyNotFound) {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
...
func PasskeyVerifyFinish(c *gin.Context) {
	...
	if err := repo.UpsertPasskeyCredential(credential); err != nil {
		common.ApiError(c, err)
		return
	}
...
func getSessionUser(c *gin.Context) (*repo.User, error) {
