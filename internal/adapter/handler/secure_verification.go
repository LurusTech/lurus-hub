...
func UniversalVerify(c *gin.Context) {
	...
	// 检查用户的验证方式
	twoFA, _ := repo.GetTwoFAByUserId(userId)
	has2FA := twoFA != nil && twoFA.IsEnabled

	passkey, passkeyErr := repo.GetPasskeyByUserID(userId)
	...
	// 记录日志
	repo.RecordLog(userId, repo.LogTypeSystem, fmt.Sprintf("通用安全验证成功 (验证方式: %s)", verifyMethod))
...
func PasskeyVerifyForSecure(c *gin.Context) {
	...
	credential, err := repo.GetPasskeyByUserID(userId)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
	...
	// 记录日志
	repo.RecordLog(userId, repo.LogTypeSystem, "Passkey 安全验证成功")
