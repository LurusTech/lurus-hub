...
func SendSmsVerification(c *gin.Context) {
	...
	if req.Purpose == "register" {
		if repo.IsPhoneAlreadyTaken(req.Phone) {
			c.JSON(http.StatusBadRequest, gin.H{
				"success": false,
...
func LoginWithSms(c *gin.Context) {
	...
	// Find user by phone
	user, err := repo.GetUserByPhone(req.Phone)
	if err != nil {
		...
		user, err = repo.CreateUserByPhone(req.Phone)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
...
func BindPhone(c *gin.Context) {
	...
	existingUser, _ := repo.GetUserByPhone(req.Phone)
	...
	// Update user phone
	if err := repo.UpdateUserPhone(userId, req.Phone); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
