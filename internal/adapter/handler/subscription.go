...
func GetCurrentSubscription(c *gin.Context) {
	...
	quotaInfo, err := repo.GetUserDailyQuotaInfo(userId)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func GetSubscriptionHistory(c *gin.Context) {
	...
	subs, err := repo.GetUserSubscriptions(userId, limit)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func CreateSubscription(c *gin.Context) {
	...
	// Get plan
	plan := repo.GetSubscriptionPlanByCode(req.PlanCode)
	if plan == nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
	...
	pendingCount, err := repo.GetUserPendingSubscriptionCount(userId)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
	...
	existingSub, err := repo.GetActiveSubscription(userId)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func CancelSubscriptionRenewal(c *gin.Context) {
	...
	if err := repo.CancelSubscription(sub.Id); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func ActivateSubscriptionByPayment(paymentId string) error {
	sub, err := repo.GetSubscriptionByPaymentId(paymentId)
	if err != nil {
		return err
	}

	return repo.ActivateSubscription(sub)
}
...
func ActivateSubscriptionById(subscriptionId int) error {
	sub, err := repo.GetSubscriptionById(subscriptionId)
	if err != nil {
		return err
	}
...
func AdminGetAllSubscriptions(c *gin.Context) {
	...
	query := repo.DB.Model(&repo.Subscription{})
	if status != "" {
...
func AdminUpdateSubscriptionPlans(c *gin.Context) {
	var plans []repo.SubscriptionPlan
	...
	if err := repo.UpdateSubscriptionPlans(plans); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func AdminExpireSubscription(c *gin.Context) {
	...
	sub, err := repo.GetSubscriptionById(id)
	if err != nil {
	...
	if err := repo.ExpireSubscription(sub); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func AdminCreateSubscription(c *gin.Context) {
	...
	existingSub, _ := repo.GetActiveSubscription(req.UserId)
	startTime := time.Now()
	if existingSub != nil {
		startTime = existingSub.ExpiresAt
	}
	...
	if err := repo.ActivateSubscription(sub); err != nil {
...
// AdminGetUserSubscriptionInfo returns user's subscription and quota info
// GET /api/subscription/admin/user/:id
func AdminGetUserSubscriptionInfo(c *gin.Context) {
	idStr := c.Param("id")
	...
	sub, _ := repo.GetActiveSubscription(userId)

	// Get subscription history
	subs, _ := repo.GetUserSubscriptions(userId, 10)

	// Get daily quota info
	quotaInfo, _ := repo.GetUserDailyQuotaInfo(userId)
...
// AdminUpdateUserRole updates user role (for admin grant/revoke subscriber)
// POST /api/subscription/admin/user/:id/role
func AdminUpdateUserRole(c *gin.Context) {
	...
	// Prevent non-root from assigning Admin/Root roles
	currentRole := c.GetInt("role")
	if req.Role >= common.RoleAdminUser && currentRole < common.RoleRootUser {
		c.JSON(http.StatusForbidden, gin.H{
			"success": false,
			"message": "Only root user can assign admin/root roles",
		})
		return
	}

	if err := repo.UpdateUserRole(userId, req.Role); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
