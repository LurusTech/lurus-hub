...
func InitiateSubscriptionPayment(c *gin.Context) {
	...
	// Get subscription
	sub, err := repo.GetSubscriptionById(req.SubscriptionId)
	if err != nil {
	...
	if sub.Status != repo.SubscriptionStatusPending {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
	...
	if time.Since(sub.CreatedAt) > 24*time.Hour {
		// Mark as expired
		_ = repo.UpdateSubscriptionStatus(sub.Id, repo.SubscriptionStatusExpired)
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
	...
	if err := repo.UpdateSubscriptionPaymentInfo(sub.Id, paymentId, req.PaymentMethod); err != nil {
...
func createStripeSubscriptionSession(sub *repo.Subscription, user *repo.User) (string, string, error) {
...
func createCreemSubscriptionSession(sub *repo.Subscription, user *repo.User) (string, string, error) {
...
func createEpaySubscriptionSession(c *gin.Context, sub *repo.Subscription, user *repo.User) (string, string, error) {
...
func handleStripeSubscriptionRefund(event stripe.Event) {
	...
	sub, err := repo.GetSubscriptionByPaymentId(charge.PaymentIntent.ID)
	if err != nil || sub == nil {
	...
	if err := repo.RefundSubscription(sub); err != nil {
...
func processSubscriptionPayment(subscriptionId int, paymentId string, paymentMethod string, amountPaid int64) error {
	...
	return repo.DB.Transaction(func(tx *repo.GormDB) error {
		...
		if sub.Status == repo.SubscriptionStatusActive {
		...
		if err := repo.ActivateSubscriptionTx(tx, &sub); err != nil {
...
func RetrySubscriptionPayment(c *gin.Context) {
	...
	// Update payment info
	_ = repo.UpdateSubscriptionPaymentInfo(sub.Id, paymentId, req.PaymentMethod)
