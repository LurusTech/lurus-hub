...
func Setup2FA(c *gin.Context) {
	...
	// 检查用户是否已经启用2FA
	existing, err := repo.GetTwoFAByUserId(userId)
	if err != nil {
	...
	// 创建备用码记录
	if err := repo.CreateBackupCodes(userId, backupCodes); err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "保存备用码失败",
		})
	...
	// 记录操作日志
	repo.RecordLog(userId, repo.LogTypeSystem, "开始设置两步验证")
...
func Enable2FA(c *gin.Context) {
	...
	// 获取2FA记录
	twoFA, err := repo.GetTwoFAByUserId(userId)
	if err != nil {
		common.ApiError(c, err)
		return
	}
	...
	// 记录操作日志
	repo.RecordLog(userId, repo.LogTypeSystem, "成功启用两步验证")
...
func Disable2FA(c *gin.Context) {
	...
	// 禁用2FA
	if err := repo.DisableTwoFA(userId); err != nil {
		common.ApiError(c, err)
		return
	}

	// 记录操作日志
	repo.RecordLog(userId, repo.LogTypeSystem, "禁用两步验证")
...
func Get2FAStatus(c *gin.Context) {
	...
	if twoFA != nil {
		...
		if twoFA.IsEnabled {
			// 获取剩余备用码数量
			backupCount, err := repo.GetUnusedBackupCodeCount(userId)
			if err != nil {
...
func RegenerateBackupCodes(c *gin.Context) {
	...
	// 记录操作日志
	repo.RecordLog(userId, repo.LogTypeSystem, "重新生成两步验证备用码")
...
func Verify2FALogin(c *gin.Context) {
	...
	// 获取2FA记录
	twoFA, err := repo.GetTwoFAByUserId(user.Id)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func AdminDisable2FA(c *gin.Context) {
	...
	// 检查目标用户权限
	targetUser, err := repo.GetUserById(userId, false)
	if err != nil {
		common.ApiError(c, err)
		return
	}
	...
	// 禁用2FA
	if err := repo.DisableTwoFA(userId); err != nil {
		if errors.Is(err, repo.ErrTwoFANotEnabled) {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "用户未启用2FA",
			})
	...
	// 记录操作日志
	adminId := c.GetInt("id")
	repo.RecordLog(userId, repo.LogTypeManage,
