...
func setupMiddlewareTestDB(t *testing.T) func() {
	...
	err = db.AutoMigrate(&repo.InternalApiKey{}, &repo.User{}, &repo.Token{}, &repo.Log{})
	if err != nil {
		t.Fatalf("failed to auto-migrate: %v", err)
	}

	oldDB := repo.DB
	oldLogDB := repo.LOG_DB
...
func TestInteg_Auth_ExpiredKey_401(t *testing.T) {
	...
	rawKey, _, err := repo.CreateInternalApiKey(
...
func TestInteg_Auth_DisabledKey_401(t *testing.T) {
	...
	// Disable the key
	err = repo.DB.Model(&repo.InternalApiKey{}).Where("id = ?", apiKey.Id).Update("enabled", false).Error
	if err != nil {
...
func TestInteg_Auth_LastUsedUpdated(t *testing.T) {
	...
	var keyBefore repo.InternalApiKey
	if err := repo.DB.First(&keyBefore, apiKey.Id).Error; err != nil {
	...
	var keyAfter repo.InternalApiKey
	if err := repo.DB.First(&keyAfter, apiKey.Id).Error; err != nil {
