...
import (
	...
	"github.com/QuantumNous/lurus-api/internal/pkg/setting/reasoning"
		...
		for _, part := range openaiContent {
			if part.Type == dto.ContentTypeText {
				...
				for {
					...
					format, base64String, err := app.DecodeBase64FileData(dataUrl)
					if err != nil {
				...
				if strings.HasPrefix(part.GetImageMedia().Url, "http") {
					...
					fileData, err := app.GetFileBase64FromUrl(c, part.GetImageMedia().Url, "formatting image for Gemini")
					if err != nil {
					...
					format, base64String, err := app.DecodeBase64FileData(part.GetImageMedia().Url)
					if err != nil {
				...
				format, base64String, err := app.DecodeBase64FileData(part.GetFile().FileData)
				if err != nil {
				...
				base64String, err := app.DecodeBase64AudioData(part.GetInputAudio().Data)
				if err != nil {
...
func geminiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response, callback func(data string, geminiResponse *dto.GeminiChatResponse) bool) (*dto.Usage, *types.NewAPIError) {
	...
	if usage.CompletionTokens <= 0 {
		...
		if len(str) > 0 {
			usage = app.ResponseText2Usage(c, responseText.String(), info.UpstreamModelName, info.GetEstimatePromptTokens())
		} else {
			usage = &dto.Usage{}
		}
	}
...
func GeminiChatHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	case types.RelayFormatClaude:
		claudeResp := app.ResponseOpenAI2Claude(fullTextResponse, info)
...
// It includes ToolUsePromptTokenCount in PromptTokens for accurate function-calling billing.
