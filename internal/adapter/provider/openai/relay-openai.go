...
func OaiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	if isAudioModel && secondLastStreamData != "" {
		...
		if err == nil && streamResp.Usage != nil && app.ValidUsage(streamResp.Usage) {
...
func OpenaiHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	if simpleResponse.Usage.PromptTokens == 0 {
		...
		if completionTokens == 0 {
			for _, choice := range simpleResponse.Choices {
				ctkm := app.CountTextToken(choice.Message.StringContent()+choice.Message.ReasoningContent+choice.Message.Reasoning, info.UpstreamModelName)
	...
	case types.RelayFormatGemini:
		geminiResp := app.ResponseOpenAI2Gemini(&simpleResponse, info)
...
func OpenaiRealtimeHandler(c *gin.Context, info *relaycommon.RelayInfo) (*types.NewAPIError, *dto.RealtimeUsage) {
	...
	case <-targetClosed:
	case err := <-errChan:
		//return app.OpenAIErrorWrapper(err, "realtime_error", http.StatusInternalServerError), nil
