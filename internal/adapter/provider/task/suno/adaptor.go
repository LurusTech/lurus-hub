...
func (a *TaskAdaptor) ValidateRequestAndSetAction(c *gin.Context, info *relaycommon.RelayInfo) (taskErr *dto.TaskError) {
	...
	if err != nil {
		taskErr = app.TaskErrorWrapperLocal(err, "invalid_request", http.StatusBadRequest)
		return
	}
	...
	if sunoRequest.ContinueClipId != "" {
		if sunoRequest.TaskID == "" {
			taskErr = app.TaskErrorWrapperLocal(fmt.Errorf("task id is empty"), "invalid_request", http.StatusBadRequest)
			return
		}
...
func (a *TaskAdaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (taskID string, taskData []byte, taskErr *dto.TaskError) {
	...
	if !sunoResponse.IsSuccess() {
		taskErr = app.TaskErrorWrapper(fmt.Errorf("%s", sunoResponse.Message), sunoResponse.Code, http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		taskErr = app.TaskErrorWrapper(err, "copy_response_body_failed", http.StatusInternalServerError)
		return
	}
