...
func TestSubscription_TenantIsolation(t *testing.T) {
	...
	tenant1 := "sub_tenant_1"
	tenant2 := "sub_tenant_2"

	// Create subscriptions in different tenants
	sub1 := &Subscription{
		UserId:   normal.Id,
		TenantId: tenant1,
		PlanCode: "monthly",
	...
	if err := CreateSubscription(sub1); err != nil {
		t.Fatalf("Create subscription1 failed: %v", err)
	}
	if err := CreateSubscription(sub2); err != nil {
		t.Fatalf("Create subscription2 failed: %v", err)
	}

	// Query should be isolated by tenant
	var t1Subs []Subscription
	DB.Where("tenant_id = ?", tenant1).Find(&t1Subs)

	if len(t1Subs) != 1 {
		t.Errorf("expected 1 subscription for tenant1, got %d", len(t1Subs))
	}
	if len(t1Subs) > 0 && t1Subs[0].PlanCode != "monthly" {
		t.Errorf("expected plan_code='monthly', got %s", t1Subs[0].PlanCode)
	}
}

func TestTopUp_TenantIsolation(t *testing.T) {
	cleanup := SetupTestDB(t)
	defer cleanup()

	_, normal, _ := SeedTestUsers(t)

	tenant1 := "topup_tenant_1"
	tenant2 := "topup_tenant_2"
	...
	topup1 := &TopUp{
		...
		Amount:        100,
		Money:         10.0,
		TradeNo:       "trade_t1_" + common.GetRandomString(8),
	...
	topup2 := &TopUp{
		...
		Amount:        200,
		Money:         20.0,
		TradeNo:       "trade_t2_" + common.GetRandomString(8),
		PaymentMethod: "creem",
		CreateTime:    common.GetTimestamp(),
		Status:        common.TopUpStatusPending,
	}

	if err := TopUpInsert(topup1); err != nil {
		t.Fatalf("Create topup1 failed: %v", err)
	}
	if err := TopUpInsert(topup2); err != nil {
		t.Fatalf("Create topup2 failed: %v", err)
	}

	// Query should be isolated by tenant
	var t1TopUps []TopUp
	DB.Where("tenant_id = ?", tenant1).Find(&t1TopUps)

	if len(t1TopUps) != 1 {
		t.Errorf("expected 1 topup for tenant1, got %d", len(t1TopUps))
	}
	if len(t1TopUps) > 0 && t1TopUps[0].Amount != 100 {
		t.Errorf("expected amount=100, got %d", t1TopUps[0].Amount)
	}
}
