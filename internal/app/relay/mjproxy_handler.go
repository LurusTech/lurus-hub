...
func RelayMidjourneyImage(c *gin.Context) {
	taskId := c.Param("id")
	midjourneyTask := repo.GetByOnlyMJId(taskId)
	...
	if channel, err := repo.CacheGetChannel(midjourneyTask.ChannelId); err == nil {
		proxy := channel.GetSetting().Proxy
		if proxy != "" {
			if httpClient, err = app.NewProxyHttpClient(proxy); err != nil {
				c.JSON(400, gin.H{
...
func RelayMidjourneyNotify(c *gin.Context) *dto.MidjourneyResponse {
	...
	midjourneyTask := repo.GetByOnlyMJId(midjRequest.MjId)
	if midjourneyTask == nil {
		return &dto.MidjourneyResponse{
			Code:        4,
	...
	err = repo.MjUpdate(midjourneyTask)
	if err != nil {
		return &dto.MidjourneyResponse{
			Code:        4,
...
func coverMidjourneyTaskDto(c *gin.Context, originTask *repo.Midjourney) (midjourneyTask dto.MidjourneyDto) {
...
func RelaySwapFace(c *gin.Context, info *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if err != nil {
		return app.MidjourneyErrorWrapper(constant.MjRequestError, "bind_request_body_failed")
	}

	info.InitChannelMeta(c)

	if swapFaceRequest.SourceBase64 == "" || swapFaceRequest.TargetBase64 == "" {
		return app.MidjourneyErrorWrapper(constant.MjRequestError, "sour_base64_and_target_base64_is_required")
	}
	modelName := app.CoverActionToModelName(constant.MjActionSwapFace)
	...
	mjResp, _, err := app.DoMidjourneyHttpRequest(c, time.Second*60, fullRequestURL)
	if err != nil {
		return &mjResp.Response
	}
	defer func() {
		if mjResp.StatusCode == 200 && mjResp.Response.Code == 1 {
			err := app.PostConsumeQuota(info, priceData.Quota, 0, true)
			if err != nil {
	...
	midjourneyTask := &repo.Midjourney{
		UserId:      info.UserId,
	...
	err = repo.MjInsert(midjourneyTask)
	if err != nil {
		return app.MidjourneyErrorWrapper(constant.MjRequestError, "insert_midjourney_task_failed")
	}
	c.Writer.WriteHeader(mjResp.StatusCode)
	respBody, err := json.Marshal(midjResponse)
	if err != nil {
		return app.MidjourneyErrorWrapper(constant.MjRequestError, "unmarshal_response_body_failed")
	}
	_, err = io.Copy(c.Writer, bytes.NewBuffer(respBody))
	if err != nil {
		return app.MidjourneyErrorWrapper(constant.MjRequestError, "copy_response_body_failed")
	}
	return nil
}
...
func RelayMidjourneyTaskImageSeed(c *gin.Context) *dto.MidjourneyResponse {
	taskId := c.Param("id")
	userId := c.GetInt("id")
	originTask := repo.GetByMJId(userId, taskId)
	if originTask == nil {
		return app.MidjourneyErrorWrapper(constant.MjRequestError, "task_no_found")
	}
	channel, err := repo.GetChannelById(originTask.ChannelId, true)
	if err != nil {
		return app.MidjourneyErrorWrapper(constant.MjRequestError, "get_channel_info_failed")
	}
	if channel.Status != common.ChannelStatusEnabled {
		return app.MidjourneyErrorWrapper(constant.MjRequestError, "该任务所属渠道已被禁用")
	}
	...
	midjResponseWithStatus, _, err := app.DoMidjourneyHttpRequest(c, time.Second*30, fullRequestURL)
	if err != nil {
	...
	app.IOCopyBytesGracefully(c, nil, respBody)
	return nil
}
...
func RelayMidjourneyTask(c *gin.Context, relayMode int) *dto.MidjourneyResponse {
	...
	case relayconstant.RelayModeMidjourneyTaskFetchByCondition:
		...
		if len(condition.IDs) != 0 {
			originTasks := repo.GetByMJIds(userId, condition.IDs)
...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if relayInfo.RelayMode == relayconstant.RelayModeMidjourneyAction { // midjourney plus，需要从customId中获取任务信息
		mjErr := app.CoverPlusActionToNormalAction(&midjRequest)
		if mjErr != nil {
			return mjErr
		}
	...
	if relayInfo.RelayMode == relayconstant.RelayModeMidjourneyImagine { //绘画任务，此类任务可重复
		if midjRequest.Prompt == "" {
			return app.MidjourneyErrorWrapper(constant.MjRequestError, "prompt_is_required")
		}
		...
		if relayInfo.RelayMode == relayconstant.RelayModeMidjourneyChange {
			if midjRequest.TaskId == "" {
				return app.MidjourneyErrorWrapper(constant.MjRequestError, "task_id_is_required")
			} else if midjRequest.Action == "" {
				return app.MidjourneyErrorWrapper(constant.MjRequestError, "action_is_required")
			} else if midjRequest.Index == 0 {
				return app.MidjourneyErrorWrapper(constant.MjRequestError, "index_is_required")
			}
			//action = midjRequest.Action
			mjId = midjRequest.TaskId
		} else if relayInfo.RelayMode == relayconstant.RelayModeMidjourneySimpleChange {
			if midjRequest.Content == "" {
				return app.MidjourneyErrorWrapper(constant.MjRequestError, "content_is_required")
			}
			params := app.ConvertSimpleChangeParams(midjRequest.Content)
			if params == nil {
				return app.MidjourneyErrorWrapper(constant.MjRequestError, "content_parse_failed")
			}
			mjId = params.TaskId
			midjRequest.Action = params.Action
		} else if relayInfo.RelayMode == relayconstant.RelayModeMidjourneyModal {
			//if midjRequest.MaskBase64 == "" {
			//	return app.MidjourneyErrorWrapper(constant.MjRequestError, "mask_base64_is_required")
			//}
			mjId = midjRequest.TaskId
		...
		originTask := repo.GetByMJId(relayInfo.UserId, mjId)
		if originTask == nil {
			return app.MidjourneyErrorWrapper(constant.MjRequestError, "task_not_found")
			...
			if setting.MjActionCheckSuccessEnabled {
				if originTask.Status != "SUCCESS" && relayInfo.RelayMode != relayconstant.RelayModeMidjourneyModal {
					return app.MidjourneyErrorWrapper(constant.MjRequestError, "task_status_not_success")
				}
			}
			channel, err := repo.GetChannelById(originTask.ChannelId, true)
			if err != nil {
	...
	modelName := app.CoverActionToModelName(midjRequest.Action)
	...
	midjResponseWithStatus, responseBody, err := app.DoMidjourneyHttpRequest(c, time.Second*60, fullRequestURL)
	if err != nil {
		return &midjResponseWithStatus.Response
	}
	midjResponse := &midjResponseWithStatus.Response
	...
	defer func() {
		if consumeQuota && midjResponseWithStatus.StatusCode == 200 {
			err := app.PostConsumeQuota(relayInfo, priceData.Quota, 0, true)
			if err != nil {
				common.SysLog("error consuming token remain quota: " + err.Error())
			}
			tokenName := c.GetString("token_name")
	...
	if midjResponse.Code == 3 {
		...
		channel, err := repo.GetChannelById(midjourneyTask.ChannelId, true)
		if err != nil {
			common.SysLog("get_channel_null: " + err.Error())
		}
		if channel.GetAutoBan() && common.AutomaticDisableChannelEnabled {
			repo.UpdateChannelStatus(midjourneyTask.ChannelId, "", 2, "No available account instance")
		}
	}
