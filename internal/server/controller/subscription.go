...
func AdminGetUserSubscriptionInfo(c *gin.Context) {
	...
	sub, _ := model.GetActiveSubscription(userId)

	// Get subscription history
	subs, _ := model.GetUserSubscriptions(userId, 10)

	// Get daily quota info
	quotaInfo, _ := model.GetUserDailyQuotaInfo(userId)
...
func AdminUpdateUserRole(c *gin.Context) {
	...
	if err := model.UpdateUserRole(userId, req.Role); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
