...
func ListUserMappingsV2(c *gin.Context) {
	...
	var mappings []*model.UserIdentityMapping
	var total int64
	var err error

	if zitadelUserID != "" {
		// List by Zitadel user across all tenants
		mappings, err = model.ListUserMappingsByZitadelUser(zitadelUserID)
		if err != nil {
		...
		if offset < len(mappings) {
			...
			mappings = []*model.UserIdentityMapping{}
		}
	} else if tenantID != "" {
		// List by tenant
		mappings, total, err = model.ListUserMappingsByTenant(tenantID, offset, pageSize)
	} else {
		// List all mappings (use system DB to bypass tenant isolation)
		err = model.GetSystemDB().Model(&model.UserIdentityMapping{}).
...
func GetUserMappingV2(c *gin.Context) {
	...
	var mapping model.UserIdentityMapping
	err = model.GetSystemDB().Where("id = ?", mappingID).First(&mapping).Error
	if err != nil {
	...
	user, err := model.GetUserById(mapping.LurusUserID, false)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"data": gin.H{
...
func DeleteUserMappingV2(c *gin.Context) {
	...
	if hardDelete {
		...
		err = model.DeleteUserMapping(mappingID)
	} else {
		// Soft delete (deactivate) the mapping
		err = model.DeactivateUserMapping(mappingID)
	}
...
func GetSystemStatsV2(c *gin.Context) {
	...
	// Count channels
	totalChannels, _ = model.CountAllChannels()

	// Count tenants
	db.Model(&model.Tenant{}).Count(&totalTenants)

	// Count user mappings
	db.Model(&model.UserIdentityMapping{}).Where("is_active = ?", true).Count(&totalMappings)

	// Count topups
	db.Model(&model.TopUp{}).Count(&totalTopUps)

	// Count subscriptions
	db.Model(&model.Subscription{}).Count(&totalSubscriptions)

	// Count redemptions
	db.Model(&model.Redemption{}).Count(&totalRedemptions)
	...
	db.Model(&model.User{}).Select("SUM(quota) as total_quota, SUM(used_quota) as used_quota").Scan(&quotaStats)

	// Get channel statistics by type
	channelsByType, _ := model.CountChannelsGroupByType()

	// Get active subscriptions count
	var activeSubscriptions int64
	db.Model(&model.Subscription{}).Where("status = ?", model.SubscriptionStatusActive).Count(&activeSubscriptions)

	// Get pending topups count
	var pendingTopUps int64
	db.Model(&model.TopUp{}).Where("status = ?", common.TopUpStatusPending).Count(&pendingTopUps)
