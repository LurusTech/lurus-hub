...
func GetTopUpsV2(c *gin.Context) {
	...
	// Get topups for user
	topups, total, err := model.GetUserTopUps(tenantCtx.UserID, pageInfo)
	if err != nil {
...
func GetSubscriptionsV2(c *gin.Context) {
	...
	subscriptions, err := model.GetUserSubscriptions(tenantCtx.UserID, limit)
	if err != nil {
	...
	activeSub, _ := model.GetActiveSubscription(tenantCtx.UserID)
...
func SubscribeV2(c *gin.Context) {
	...
	if err := model.CreateSubscription(subscription); err != nil {
