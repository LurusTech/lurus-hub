...
func ListChannelsV2(c *gin.Context) {
	...
	if keyword != "" || group != "" || modelFilter != "" {
		// Use search function
		allChannels, searchErr := model.SearchChannels(keyword, group, modelFilter, idSort)
		if searchErr != nil {
		...
		// Filter by tag
		allChannels, tagErr := model.GetChannelsByTag(tag, idSort, false)
		if tagErr != nil {
		...
		var getAllErr error
		channels, getAllErr = model.GetAllChannels(startIdx, pageSize, false, idSort)
		if getAllErr != nil {
...
func UpdateChannelV2(c *gin.Context) {
	...
	existingChannel, err := model.GetChannelById(channelID, true)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{
			"success": false,
			"message": "Channel not found",
