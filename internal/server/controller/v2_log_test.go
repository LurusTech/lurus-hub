...
func TestGetLogsV2_UserScope(t *testing.T) {
	...
	SeedV2Log(t, ctx, ctx.AdminUser.Id, model.LogTypeConsume)
...
func TestGetLogsV2_Filters(t *testing.T) {
	...
	// Filter by type
	w := V2RequestAsUser(ctx, ctx.NormalUser, http.MethodGet, "/api/v2/test-tenant/logs?type="+strconv.Itoa(model.LogTypeConsume), nil, nil)
...
func TestGetAllLogsV2_WithFilters(t *testing.T) {
	...
	// Filter by type only
	w := V2RequestAsUser(ctx, ctx.AdminUser, http.MethodGet, "/api/v2/test-tenant/logs/all?type="+strconv.Itoa(model.LogTypeConsume), nil, []string{"admin"})
