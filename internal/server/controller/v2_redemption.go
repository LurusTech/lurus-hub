...
func RedeemCodeV2(c *gin.Context) {
	...
	// Redeem the code
	quota, err := model.Redeem(req.Code, tenantCtx.UserID)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": err.Error(),
...
func ListRedemptionsV2(c *gin.Context) {
	...
	if keyword != "" {
		// Search redemptions
		redemptions, total, err = model.SearchRedemptions(keyword, startIdx, pageSize)
	} else {
		// Get all redemptions
		redemptions, total, err = model.GetAllRedemptions(startIdx, pageSize)
	}
