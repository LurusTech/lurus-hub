...
func V2RequestAsUser(ctx *V2TestContext, user *model.User, method, path string, body interface{}, roles []string) *httptest.ResponseRecorder {
	headers := map[string]string{
		"X-Test-Tenant-ID": ctx.TenantID,
...
func SeedV2Token(t *testing.T, ctx *V2TestContext, userID int, name string) *model.Token {
	t.Helper()
	token := &model.Token{
		UserId:         userID,
...
func SeedV2Channel(t *testing.T, ctx *V2TestContext, name string) *model.Channel {
	t.Helper()
...
func SeedV2Subscription(t *testing.T, ctx *V2TestContext, userID int, status string) *model.Subscription {
	t.Helper()
	now := time.Now()
...
func SeedV2TopUp(t *testing.T, ctx *V2TestContext, userID int, status string) *model.TopUp {
	t.Helper()
	topup := &model.TopUp{
		UserId:        userID,
		TenantId:      ctx.TenantID,
...
func SeedV2Redemption(t *testing.T, ctx *V2TestContext, userID int) *model.Redemption {
	t.Helper()
...
func SeedV2Log(t *testing.T, ctx *V2TestContext, userID int, logType int) *model.Log {
	t.Helper()
	log := &model.Log{
		UserId:    userID,
		TenantId:  ctx.TenantID,
