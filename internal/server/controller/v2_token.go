...
func ListTokensV2(c *gin.Context) {
	...
	tokens, err := model.GetAllUserTokens(tenantCtx.UserID, offset, pageSize)
	if err != nil {
	...
	// Get total count
	total, _ := model.CountUserTokens(tenantCtx.UserID)
...
func CreateTokenV2(c *gin.Context) {
	...
	// Validate token name
	if err := service.ValidateTokenName(req.Name); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": err.Error(),
		})
	...
	// Validate quota
	if err := service.ValidateTokenQuota(req.RemainQuota, req.UnlimitedQuota); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": err.Error(),
		})
...
func UpdateTokenV2(c *gin.Context) {
	...
	token, err := model.GetTokenByIds(tokenID, tenantCtx.UserID)
	if err != nil {
	...
	if req.Status != nil {
		if *req.Status == common.TokenStatusEnabled {
			if err := service.CanEnableToken(token); err != nil {
				c.JSON(http.StatusBadRequest, gin.H{
					"success": false,
					"message": err.Error(),
				})
...
func DeleteTokenV2(c *gin.Context) {
	...
	// Delete token
	err = model.DeleteTokenById(tokenID, tenantCtx.UserID)
	if err != nil {
